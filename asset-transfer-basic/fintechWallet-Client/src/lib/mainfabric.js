...
const channelName = 'finchannel';
const chaincodeName = 'finchannelbs';
...
const getAllBlockchainData = async () => {
    try {
...
export const submitDataToBlockchain = async (fintechWalletData) => {
    try {

       
        const contract = initiateContract();
        await contract.submitTransaction('CreateAsset', JSON.stringify(fintechWalletData));
        console.log('*** Result: committed');
    } catch (error) {
        console.error(`*Error Occured while trying to run the application: ${error}`);
    }
...
const updateDataBlockchain = async(payrollData) => {
    ...
    const updatAsset =  {
        ...
        AccountCurrency: 'GBP',
        AccountLastTransaction: 1250,
        AccountTotalTransaction: 12622,
        TotalTransaction: 212,
